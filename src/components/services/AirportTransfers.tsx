...
const arrivalSteps: Step[] = [
  { icon: AirplaneLanding, title: 'Flight monitoring', text: 'Share your flight number so your pick-up follows the live arrival time.' },
  { icon: IdentificationCard, title: 'A personal welcome', text: 'After baggage claim, meet your driver in Arrivals with your name displayed.' },
  { icon: Car, title: 'A direct transfer', text: 'Continue privately to your destination with luggage assistance.' },
]
const departureSteps: Step[] = [
  { icon: AirplaneTakeoff, title: 'Departure details', text: 'Share the airport, flight number, departure time and pick-up address.' },
  { icon: Clock, title: 'A planned collection', text: 'Your pick-up time and accessible meeting point are agreed before travel.' },
  { icon: Car, title: 'Terminal drop-off', text: 'Travel directly to the agreed terminal with luggage assistance.' },
]
const addressSteps: Step[] = [
  { icon: MapPin, title: 'Your journey details', text: 'Share both addresses, the travel date and your preferred pick-up time.' },
  { icon: Clock, title: 'An accessible pick-up', text: 'Meet at the address or the nearest point your vehicle can reach.' },
  { icon: Car, title: 'Door-to-door comfort', text: 'Travel directly to your destination with luggage assistance.' },
]

const stepVariants = [arrivalSteps, departureSteps, addressSteps]
const meetingTitles = ['Your meeting point', 'Your airport drop-off', 'An accessible pick-up']
const meetingDescriptions = [
  'See where to meet your driver after baggage claim.',
  'Your terminal and collection time are confirmed from your flight details.',
  'Share any access restrictions so we can confirm the closest meeting point.',
]
...
export default function AirportTransfers({ onMeetingPoint, onRequest }: { onMeetingPoint: () => void; onRequest: RequestJourney }) {
  ...
  const process = <div className="airport-process-layout">
    <div>
      ...
      <p className="airport-waiting-note">Your quote states the included waiting time and any charge that may apply.</p>
    </div>
    <aside className="airport-meeting">
      ...
      <button type="button" className={`sv-button${arriving ? '' : ' airport-reserved-control'}`} disabled={!arriving} aria-hidden={!arriving} onClick={onMeetingPoint}>View meeting point <ArrowRight size={21} aria-hidden="true" /></button>
      <p className="airport-meeting-note">Final instructions are sent before travel.</p>
    </aside>
  </div>
  ...
  return <section id="service-airport" className="airport-transfers" aria-labelledby="airport-transfers-title">
    <div className="svc-shell airport-content">
      <p className="svc-eyebrow">Venice airport transfers</p>
      <h2 id="airport-transfers-title">A clear pick-up. A private journey.</h2>
      <p className="airport-intro">From flight monitoring to the Arrivals meeting point, each detail is agreed before you travel.</p>
      ...
      <button type="button" className="sv-text-link airport-request" onClick={() => onRequest({ service: 'airport', airportPickup: arriving })}>Request a private transfer <ArrowRight size={18} aria-hidden="true" /></button>
    </div>
  </section>
}
